...
const Card = ({ source, titulo, minitext, ruta }) => {
    ...
    >
      ...
      <div className='h-full'>
        <div className='h-5/6 border-b-2 flex justify-end'>
          <div className='sm:w-full w-4/6 lg:text-4xl sm:h-1/2 h-full text-med text-grisoscuro px-5 lg:py-0 py-2 self-end'>
            {titulo}
          </div>
        </div>
        <div className='h-1/6 flex items-center'>
          ...
          <div className='pl-2 text-grisclaro font-normal text-xs'>{minitext}</div>
        </div>
      </div>
    </div>
  );
